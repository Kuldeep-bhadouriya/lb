...
export default function PickupSearch() {
  ...
  const handleSelect = (data: any) => {
    try {
      ...
      if (!lat || !lng) {
        Alert.alert("Error", "Could not retrieve coordinates. Try again.");
        return;
      }
  ...
  const confirmSelection = async () => {
    ...
    router.replace({
      pathname: targetHome,
      params: {
        pickupDesc: encodeURIComponent(desc),
        pickupLat: String(lat),
        pickupLng: String(lng),
        pickupIsCurrent: "1",
        destinationDesc: normalize(params.destinationDesc),
        destinationLat: normalize(params.destinationLat),
        destinationLng: normalize(params.destinationLng),
      },
    });
  };
  ...
  const confirmMapLocation = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert("Permission denied", "Enable location access to confirm.");
        return;
      }
